...
from .geometry import Collision, detect_collisions
from .schemas import (
    CircleOut,
    CollisionOut,
    PointOut,
...
def precheck(payload: PrecheckRequest) -> PrecheckResponse:
    ...
    raw: List[Collision] = detect_collisions(
        nodes=nodes,
        circles=circles,
    ...
    collisions = [
        CollisionOut(
            ...
            nearest=PointOut(x=round3(c.nearest[0]), y=round3(c.nearest[1])),
            distance=round3(c.distance),
            expanded_radius=round3(c.expanded_radius),
            circle_center=PointOut(x=round3(circles[c.circle_index][0][0]),
                                   y=round3(circles[c.circle_index][0][1])),
